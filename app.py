...
def default_holdings() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {'ticker': 'NVDA', 'qty': 10.0, 'avg_cost': 650.0},
            {'ticker': 'AAPL', 'qty': 25.0, 'avg_cost': 175.0},
            {'ticker': 'BTC-USD', 'qty': 0.25, 'avg_cost': 45000.0},
        ]
    )
...
st.caption('Edit holdings live. Move scenarios up to 400%. Watch allocations shift.')

if 'holdings' not in st.session_state:
    st.session_state.holdings = normalize_df(default_holdings())
...
with top_a:
    if st.button('Add row'):
        st.session_state.holdings = pd.concat(
            [st.session_state.holdings, pd.DataFrame([{'ticker': '', 'qty': 0.0, 'avg_cost': 0.0}])],
            ignore_index=True
        )
...
with top_c:
    ...
    if uploaded is not None:
        try:
            ...
            st.session_state.holdings = normalize_df(imported)
            st.success('Imported holdings')
        except Exception:
            st.error('Import failed. CSV needs columns: ticker, qty, avg_cost')

with top_d:
    export_csv = st.session_state.holdings.to_csv(index=False).encode('utf-8')
    st.download_button(
...
edited = st.data_editor(
    ...
    column_config={
        ...
        'qty': st.column_config.NumberColumn('Qty', step=1.0),
        'avg_cost': st.column_config.NumberColumn('Avg cost', step=0.01),
    },
)

df = normalize_df(edited)
...
try:
    ...
    with p1:
        ...
        if len(alloc_now):
            ...
            st.info('Add holdings and prices to see allocation')
    ...
    st.warning('Plotly is missing. Add plotly to requirements.txt')
...
st.caption('This is a scenario dashboard. Live pricing is best effort and may be rate limited.')
